...
def negamax_alpha_beta node, height, reorder=false, a=-INFINITY, h=INFINITY, count=0
	...
	_return = lambda {|r, pv| [r, pv, count]}
	if reorder
		node.reorder
	end
...
def iterative_negamax_alpha_beta node,height,reorder=false,a=-INFINITY,h=INFINITY,count=0
	pv = []
	(0..height).each do |_h|
		a, pv, count = negamax_alpha_beta node, _h, reorder, a, h, count
	end
	[a, pv, count]
end
...
def pvs node, depth, reorder=false, alpha=-INFINITY, beta=INFINITY, count=0
	...
	if depth == 0 or node.children.length == 0
		...
		score, pv, count = 
		neg.call pvs node.daughters[0], depth-1, reorder, -beta, -alpha, count
		...
		if score < beta
			node.daughters[1..-1].each do |daughter|
				...
				ub, _pv = lb+1, []
				temp, _pv, count = 
				neg.call pvs daughter, depth-1, reorder, -ub, -lb, count

				if temp >= ub and temp < beta
					temp, _pv, count = 
					neg.call pvs daughter, depth-1, reorder, -beta, -temp, count
				end
...
def iterative_pvs node, depth, reorder=false, alpha=-INFINITY, beta=INFINITY, count=0
	score, pv = 0, []
	(0..depth).each do |_h|
		score, pv, count = pvs node, _h, reorder, alpha, beta, count
	end
	[score, pv, count]
end
